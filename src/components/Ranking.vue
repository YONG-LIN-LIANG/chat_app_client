...
<template>
	<div class="ranking_section mt-10 ml-48 lg:ml-36 sm:ml-0 sm:mt-[104px]">
		...
		<div class="ranking_container min-h-screen p-7 sm:flex sm:flex-col sm:items-center">
			<div class="ranking_wrap_top mb-6">
				<div
					v-for="(rateItem, idx) in ratingList"
					:key="rateItem.name"
					:class="['h-14 flex items-center mb-1', { 'mb-3.5': idx < 2, 'mb-6': idx === 2 }]"
				>
					<div
						class="ranking_rank w-14 min-w-14 h-14 flex items-center justify-center relative text-gray-2 mr-2"
					>
						<SunIcon v-if="idx < 3" />
						<span class="absolute top-1/2 left-1/2 translate-x-1/2Re translate-y-1/2Re leading-12">{{
							idx + 1
						}}</span>
					</div>
