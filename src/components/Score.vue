...
export default {
	...
	setup() {
		...
		return { comments, commentsCount, ratingCount }
	},
}
</script>
<template>
			...
			<div class="score_container w-80">
				<div class="score_filter_section flex flex-col items-center px-10 py-3">
					<div
						...
						:key="item.star"
						:class="[
							{ 'bg-green-w20': item.star === 5 },
							'ranking_star_wrap w-56 flex items-center mr-5 px-4 py-2 rounded-20',
						]"
					>
						<div class="ranking_star flex text-green-Default mr-2.5">
							<StarIcon v-for="i in 5" :key="i" :class="[{ 'text-gray-6': item.star < i }, 'mr-1']" />
						</div>
						<div class="ranking_average text-green-Default mr-5">{{ item.star }}</div>
				...
				<div class="search_filter_section px-10 py-3">
					<div class="search_filter_title h-10 flex items-center justify-between">
					...
					<p class="text-green-b50 my-4 text-center cursor-pointer">清除所有篩選條件</p>
				</div>
			</div>
				...
				<div class="comment_list border border-green-neon rounded-xl p-4">
					<div v-for="(item, idx) in comments" :key="idx" class="comment p-3">
						<div class="flex items-center justify-between sm:block">
							<div class="flex items-center">
								<div class="comment_name w-16 text-gray-3 text-sm">{{ item.name }}</div>
								<span class="chat_source whitespace-nowrap"> {{ item.website }} </span>
							</div>
							<div class="comment_date text-xs text-gray-3"> {{ item.created_date }}</div>
						</div>
