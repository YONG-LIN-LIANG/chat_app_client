...
export default {
	...
	setup() {
			...
			data: [
				{
					...
					chatList: [
						...
						{ status: 1, name: '陳大倫', msg: '有啥問題', isRead: true, created_time: '2022-05-03 14:04' },
						{
							status: 2,
							name: 'Molly',
							msg: '您好想請問您貴姓',
						...
						{ status: 1, name: '陳大倫', msg: '敝姓張', isRead: false, created_time: '2022-05-03 14:40' },
					],
				},
				{
					roomId: '2',
		...
		SourceList.data.forEach(function (i) {
			...
			i['startDate'] = i.chatListAll[0].created_time.split(' ')[0].split('-').join('/')
		})

		return { DoingIcon, SourceList }
	},
}
</script>
<template>
	<div class="chat_section px-10 lg:px-7 py-7 flex items-center flex-col overflow-auto box-border">
		<div v-for="item in SourceList.data" :key="item.roomId" class="w-full flex items-center flex-col" data-part="0">
			<div class="chat_date w-16 relative text-xs font-light text-center text-gray-3 mx-0 my-5">
				{{ item.startDate }}
			</div>
			<div class="chat_start">
				<div class="chat_status text-sm font-medium text-green-Default text-center mx-0 my-1.5">開始聊天</div>
				<div class="chat_source_wrap text-sm mx-0 my-5 text-center">
					<span class="text-sm text-gray-3 mr-1">來源 : </span>
					<span class="chat_source inline-block text-gray-2 bg-orange-w20 m-1 px-3.5 py-1">{{
						item.group
					}}</span>
					<span class="chat_source inline-block text-gray-2 bg-orange-w20 m-1 px-3.5 py-1">{{
						item.website
			...
			<div v-for="chatItem in item.chatListAll" :key="chatItem.created_time" class="w-full mx-0 my-3">
				<div v-if="chatItem.status === 0" class="chat_tags_wrap">
					<div class="chat_tags_ques text-xs text-gray-2 text-center mx-0 my-2.5">{{
						chatItem.question
					}}</div>
					<div class="chat_tags_opts_wrap flex justify-center">
						<div
							v-for="optItem in chatItem.optionList"
							:key="optItem"
							:class="[
								{ 'bg-green-w50': optItem === chatItem.answer },
				...
				<div v-if="chatItem.status === 2" class="msg_receive">
					<span class="msg_receive_text bg-gray-6">{{ chatItem.msg }}</span>
					<span class="msg_time text-xs font-light text-gray-3 tracking-wide mx-2 my-0.5">{{
						chatItem.created_time.split(' ')[1]
					}}</span>
				</div>
				<div v-if="chatItem.status === 1" class="msg_send justify-end">
					...
					<span class="msg_send_text bg-green-w50">{{ chatItem.msg }}</span>
				</div>
			</div>

			<div class="chat_end">
				<div class="chat_status text-sm font-medium text-green-Default text-center mx-0 my-1.5">結束聊天</div>
