<template>
	<div class="flex items-center flex-col px-5 pt-5 box-border">
		<div class="w-full flex items-center flex-col">
			...
			<div class="text-sm font-medium text-green-Default text-center mx-0 my-1.5"> 開始聊天 </div>
			<!-- v-for="(item, idx) in messageInfo.chatList" :key="idx" -->

			<div v-for="(item, idx) in messageInfo.chatList" :key="idx" class="w-full">
				<div v-if="item.status === 0 && item.question_id === 1" class="mx-0 my-2.5">
					<div class="text-xs text-gray-2 text-center mx-0 my-2.5"> 請選擇想詢問的問題 </div>
				...
				<div v-if="item.status === 0 && item.question_id === 2" class="mx-0 my-2.5">
					<div class="text-xs text-gray-2 text-center mx-0 my-2.5"> 請選擇服務的客服人員 </div>
					<div class="chat_tags_opts_wrap flex justify-center">
						<div
							v-for="(option, idx2) in item.optionList"
							:key="idx2"
				...
				<!-- 人員聊天跑回圈 -->
				<div v-if="item.status === 1 || item.status === 2" class="w-full mx-0 my-2.5">
					<div v-if="item.status === 2" class="message w-full flex items-end mr-7">
						<span class="text-md text-gray-1 bg-gray-6 tracking-wide rounded-md px-4 py-2">{{
							item.msg
						}}</span>
						<span class="text-xs font-light text-gray-3 tracking-wide mx-2 my-0.5">{{
							formatTime(item.created_time)
						}}</span>
					</div>
					<div v-if="item.status === 1" class="message w-full flex items-end justify-end mr-7">
						<span class="text-xs font-light text-gray-3 tracking-wide mx-2 my-0.5">{{
							formatTime(item.created_time)
						}}</span>
						<span class="text-md text-gray-1 bg-green-w50 tracking-wide rounded-md px-4 py-2">{{
							item.msg
						}}</span>
					</div>
				</div>
			...
			<div class="mb-5">
				<div class="text-sm font-medium text-green-Default text-center mx-0 my-1.5"> 結束聊天 </div>
