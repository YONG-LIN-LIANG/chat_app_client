<template>
  <section :class="['room-window-container fixed right-0 bottom-0', isInRoom ? 'h-full max-h-141 pt-16' : 'h-auto pt-0']">
    <!-- 聊天室 -->
    <div :class="['room-window flex flex-col relative w-85 h-full bg-white rounded-md overflow-hidden shadow-layer2', isRoomOpen ? 'room-window--active' : '']">
      <div class="flex-none flex justify-between items-center px-5 py-2.5 bg-green-w20">
        <h4>即時客服</h4>
        <div class="flex items-center">
          <button class="header-icon"><SearchIcon class="text-gray-3" /></button>
          <button @click="handleToggleRoom" class="header-icon"><HideIcon  /></button>
          <button @click="handleOpenDialog('end')" class="header-icon"><CrossIcon  /></button>
        </div>
      </div>
      <div v-show="isInRoom" class="flex-grow overflow-y-auto">
        <MsgChat v-for="(item, idx) in formatChatList" :key="idx" :messageInfo="item" :roomInfo="roomInfo" />
      </div>
      ...
      <div v-show="isLoginReminderDisplay" class="text-center py-7.5">
        ...
        <button @click="visitorStep = 2" class="block mt-7.5 px-2.5 py-1.5 mx-auto bg-gray-6 rounded-md">跳過登入</button>
      </div>
      <!-- 未登入輸入姓名 -->
      <div v-show="isFillClientNameDisplay" class="flex flex-col p-7.5 px-11 text-center">
        <h5 class="text-xs">請輸入姓名</h5>
        <div class="relative mt-5">
          <input v-model="visitorNameForm.name" type="text" :class="['w-full', isVisitorNameFilled ? 'border-red' : '']">
          <span v-show="!visitorNameForm.name && visitorNameForm.errorMessage" class="absolute left-0 -bottom-5 text-xs text-red">{{ visitorNameForm.errorMessage }}</span>
        </div>
        
        <button @click="handleVisitorJoinRoom" class="w-35 h-9 mx-auto mt-10 bg-green-Default rounded-full">下一步</button>
      </div>
      <!-- 結束聊天燈箱 -->
      <CloseRoomDialog 
      :class="['endDialog absolute left-0 top-12 right-0 w-full', dialog.name === 'end' && dialog.isOpen ? 'endDialog-active' : '']" 
      @onCloseDialog="handleCloseDialog"
      />
    </div>
  </section>
  <!-- 球+新訊息提醒 -->
  <section :class="['chat-ball flex justify-end items-center fixed right-0 bottom-6', isRoomOpen ? 'chat-ball--active' : '']">
    <div class="shrink-0 flex items-center py-2.5 px-5 border border-green-Default rounded-10 shadow-layer2 bg-white">
      客服人員 {{  }} 在線為您服務
    </div>
    <div @click="handleToggleRoom" class="ml-5 p-3 border-2 border-green-neon rounded-full bg-white shadow-layer2 cursor-pointer">
      <CsAvatar />
    </div>
  </section>
</template>
      ...
      const roomInfo = reactive({
        ...
        chat: [
          ...
          chatList: [
            ...
            { status: 1, name: '陳大倫', msg: '有啥問題', isRead: true, created_time: '2022-06-05 14:02' },
            {
            status: 2,
            name: 'Molly',
            msg: '您好想請問您貴姓',
            ...
            { status: 1, name: '陳大倫', msg: '敝姓張', isRead: false, created_time: '2022-06-05 14:04' },
          ],
          },
          {
          roomId: '2',
          ...
          chatList: [
            ...
            { status: 1, name: '陳大倫', msg: '有啥問題', isRead: true, created_time: '2022-06-06 14:02' },
            {
            status: 2,
            name: 'Molly',
            msg: '您好想請問您貴姓',
            ...
            { status: 1, name: '陳大倫', msg: '敝姓張', isRead: false, created_time: '2022-06-06 14:04' },
          ],
        },
    ...
    computed: {
      ...
      formatChatList() {
        ...
        const chatList = chat.map(i => {
          ...
          newChatList = newChatList.sort((a, b) => a.timeStampDate-b.timeStampDate)
          const {roomId, group, website, csName} = i
          const newObj = {roomId, group, website, csName, chatList: newChatList}
          return newObj
        })
        return chatList
      }
